          ...
          <>
            ...
            <div className="hero-grid">
              <div className="hero-card">
                {hero.image
                  ? <img src={hero.image} alt={hero.title} style={{ width: '100%', height: 220, objectFit: 'cover', display: 'block' }} />
                ...
                {sideArticles.map(a => (
                  <div className="side-card" key={a.id}>
                    <div className="side-thumb">
                      {a.image
                        ? <img src={a.image} alt={a.title} style={{ width: 72, height: 60, objectFit: 'cover', borderRadius: 4 }} />
                        : a.icon
                      }
                    </div>
                    <div>
                    ...
                    <div className="cat-items">
                      {items.length ? items.map(a => (
                        <Link href={`/article/${a.slug}`} key={a.id}>
                          <div className="cat-item">{a.title}<span>{timeAgo(a.date)}</span></div>
                        </Link>
                      )) : (
